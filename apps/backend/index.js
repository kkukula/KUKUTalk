...
const chatRt = require('./routes/chat')
...
app.use(hpp())
app.options('*', cors({ origin: corsOrigin, credentials: false, methods: ['GET','POST','OPTIONS'], allowedHeaders: ['Authorization','Content-Type'] }))
app.use(compression())
...
app.use(cors({ origin: corsOrigin, credentials: false, methods: ['GET','POST','OPTIONS'], allowedHeaders: ['Authorization','Content-Type'] }))
...
// Chat history
app.get('/chat/history', chatRt.history)
// Profiles and parental controls
...
io.on('connection', (socket) => {
  ...
  socket.on('chat:message', (msg) => {
    ...
    try { messages.add(safe, process.env.HISTORY_MAX || 200) } catch(e) {}
