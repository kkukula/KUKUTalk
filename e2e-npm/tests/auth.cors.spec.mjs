...
const API="http://localhost:3001", PATH="/auth/login";
...
for (const port of [5173,5174]){
  test(`CORS+AUTH on http://localhost:${port}`, async ({page})=>{
    const origin=`http://localhost:${port}`;
    const a=await doFetch(page,origin,{username:"x",password:"x"});
    expect(a.ok).toBeTruthy(); expect([400,401,403]).toContain(a.status);
    const b=await doFetch(page,origin,{email:"nobody@example.com",password:"x"});
    expect(b.ok).toBeTruthy(); expect([400,401]).toContain(b.status);
  });
}
